...
namespace RedBubble.Application.Services
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<IReadOnlyList<Order>> GetAllAsync()
        {
            ...
            foreach (var order in orders.Take(5)) // Log first 5 orders
            {
                Console.WriteLine($"Order {order.Id}: Status = {order.Status}, Created = {order.CreatedOn}, Modified = {order.LastModifiedOn}");
            }
        ...
        public async Task<bool> ChangeStatus(int orderId, OrderStatus status)
        {
            try
            {
                // Try repository update approach
                var orderRepo = _unitOfWork.GetRepository<Order, int>();
                
                Console.WriteLine($"Attempting to update order {orderId} to status {status}");
                
                // Get the current order to verify it exists
                var existingOrder = await orderRepo.GetAll()
                    .AsNoTracking()
                    .FirstOrDefaultAsync(o => o.Id == orderId);
                ...
                Console.WriteLine($"Found order {orderId} with current status: {existingOrder.Status}");

                // Try the repository update method
                var order = await orderRepo.GetAll()
                    .FirstOrDefaultAsync(o => o.Id == orderId);
                ...
                {
                    order.Status = status;
                    order.LastModifiedBy = _currentUserService.UserId ?? "admin-001";
                    order.LastModifiedOn = DateTime.UtcNow;
                    
                    orderRepo.Update(order);
                    var result = await _unitOfWork.CompleteAsync();
                    
                    Console.WriteLine($"Repository update result: {result} rows affected");
                    
                    if (result > 0)
                    {
                        // Verify the change
                        var refreshed = await orderRepo.GetAll()
                            .AsNoTracking()
                            .FirstOrDefaultAsync(o => o.Id == orderId);
                        
                        var success = refreshed != null && refreshed.Status == status;
                        Console.WriteLine($"Verification: Order status is now {refreshed?.Status}, expected {status}, success: {success}");
                        return success;
                    }
                }

                Console.WriteLine("Repository update failed, trying alternative approach...");
                return false;
            }
            catch (Exception ex)
            {
