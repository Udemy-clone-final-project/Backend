...
namespace RedBubble.WebAPI.Controllers
{
    ...
    public class ProductVariantController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllProductVariants(string? searchItem, string? sortColumn, string? sortOrder
            , int? Category, int page = 1, int pageSize = 10)
        {
            if (page <= 0) page = 1;
            if (pageSize <= 0) pageSize = 10;

            var products = await _serviceManager.productVariantService.GetAllProductVariantsAsync( searchItem, sortColumn, sortOrder,  Category,  page,  pageSize);
            return Ok(products);     
        }
